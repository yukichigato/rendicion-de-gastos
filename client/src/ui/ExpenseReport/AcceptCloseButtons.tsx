...
const AcceptCloseButtons = ({ status }: { status: string }) => {
  if (status === "Pending") {
    return (
      <>
        <button className="rounded-lg border-[.0626rem] border-red-400 bg-red-400 p-2 text-white transition-all duration-200 hover:border-gray-900 hover:bg-white hover:text-gray-900">
          Accept
        </button>
